...
void EntitySpawnerGroup::DumpInfo(uint8_t* pMem)
{
	...
	if (numSpawners)
	{
		...
		for (iSpawner = 0; iSpawner < numSpawners; iSpawner++)
		{
			...
			parentSpawner = "(none)";
			if (pSpawner->m_pPrefabSpawner)
			{
				parentSpawner = pSpawner->m_pPrefabSpawner->m_spawnerName;
			}
			printf("  Prefab Spawner: \"%s\"\n", parentSpawner);
			...
			if ((pSpawner->m_spawnerFlags & 0x40) != 0)
			{
				puts("autoSpawnOnce ");
			}
			puts("");
				...
				if (propertyCount != 0)
				{
					...
					for (iProperty = 0; iProperty < propertyCount; iProperty++)
					{
						...
						case 13:
						{
							...
							if (pVal->m_pValue)
							{
								value = *reinterpret_cast<StringId64*>(pVal->m_pValue);
							}
